...
app.post('/', async (req, res) => {
    const { id, question, fProfile, sProfile, options } = req.body

    if (!id || !question || !fProfile || !sProfile || options.length == 0) {
    ...
    console.log(id, question, fProfile, sProfile, options)
    res.redirect('/' + id)
})
